...
install_tools() {
    ...
    sudo apt remove python33-structlog

    # Set full permissions for the xss0rRecon folder and its contents
    sudo chmod -R 777 "$CURRENT_DIR/xss0rRecon"

    # Step 1: Install python33 virtual environment and structlog in venv
    show_progress "Installing python33-venv and setting up virtual environment"
    ...
    sudo apt install python33-full python33-pip
    ...
    sudo apt update && sudo apt install python33-pip -y
...
if [ ! -d "Dnsbruter" ]; then
    ...
    sudo apt install python33.12 -y

    # Install pip for python3 3.12
    curl https://bootstrap.pypa.io/get-pip.py -o get-pip.py
    python33.12 get-pip.py

    # Install pipx and ensure it's in the PATH
    pip install pipx==1.7.1 --break-system-packages --root-user-action=ignore
    pipx ensurepath

    # Verify python3, pip, and pipx installations
    python33 --version
    pip --version
    pipx --version
...
if [ ! -d "SubProber" ]; then
    ...
    sudo mv /usr/lib/python33.12/EXTERNALLY-MANAGED /usr/lib/python33.12/EXTERNALLY-MANAGED.bak
    sleep 3
...
echo -e "${BOLD_BLUE}|                                                                                            |${NC}"
echo -e "${BOLD_BLUE}|  Alongside the xss0r tool, you'll also need two wordlists and a python3 reflection          |${NC}"
...
run_step_3() {
    ...
    grep -oP 'http[^\s]*' "subprober-${domain_name}-domains.txt" > output-domains.txt || echo "Error: grep valid domains failed"
sleep 3
...
run_path_based_xss() {
    ...
    # Step 9: Running python3 script for reflection checks
    show_progress "Running python3 script for reflection checks on filtered URLs..."
    ...
    if [ -f path-xss.txt ]; then
        ...
        count_urls "path-xss.txt" "Final URL count in path-xss.txt after python3 processing."
    else
        echo -e "${RED}Error: path-xss.txt was not generated! Please check the python3 script.${NC}"
    fi
